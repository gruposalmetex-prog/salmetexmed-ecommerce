...
import { createAreaImageUploadSignature } from "../actions/create-area-image-upload-signature.action";

import {
  saveAdminAreaImageAction,
  type SaveAdminAreaImageState,
} from "../actions/save-admin-area-image.action";

import {
  deleteAdminAreaImageAction,
  type DeleteAdminAreaImageState,
} from "../actions/delete-admin-area-image.action";

interface AdminAreaImageSectionProps {
  areaId: string;
  areaName: string;
...
const initialSaveState: SaveAdminAreaImageState = {
  status: "idle",
};

const initialDeleteState: DeleteAdminAreaImageState = {
  status: "idle",
};
...
export function AdminAreaImageSection({
  areaId,
  areaName,
  imageUrl,
  imagePublicId,
}: AdminAreaImageSectionProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const signatureResult = await createAreaImageUploadSignature({
        areaId,
      });
      ...
      actionFormData.set("areaId", areaId);
            ...
            {imageUrl ? (
              ...
              <div className="px-6 text-center">
                ...
                <p className="mt-3 text-sm font-medium text-slate-500">
                  El área todavía no tiene imagen.
                </p>
              </div>
            )}
          </div>
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              El área quedará sin imagen representativa.
            </p>
          </div>
        ...
        <form action={deleteAction} className="px-5 py-4">
          ...
          <p className="text-sm text-slate-700">
            ¿Quieres eliminar la imagen de <strong>{areaName}</strong>?
          </p>
