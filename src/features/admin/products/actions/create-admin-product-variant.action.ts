...
export interface CreateAdminProductVariantState {
  status:
    | "idle"
    | "error";
...
export async function createAdminProductVariantAction(
  _previousState:
    CreateAdminProductVariantState,

  formData: FormData,
): Promise<CreateAdminProductVariantState> {
  await requireAdmin();
  ...
  const result =
    createAdminProductVariantSchema.safeParse({
      productId:
  ...
  try {
    ...
    if (databaseError) {
      ...
      return {
        ...
        message:
          "El SKU o código de barras ya está registrado.",
      };
    }
    ...
    return {
      ...
      message:
        "No fue posible guardar la variante. Inténtalo nuevamente.",
    };
  }
  ...
  redirect(
    `/admin/productos/${productId}/editar?step=${nextStep}&variantCreated=1`,
  );
}