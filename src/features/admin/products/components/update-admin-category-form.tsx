...
import { LoaderCircle, RefreshCw, Save, Tags } from "lucide-react";
import { useActionState, useState } from "react";

import {
  updateAdminCategoryAction,
  type UpdateAdminCategoryState,
} from "../actions/update-admin-category.action";

interface AdminCategoryAreaOption {
  id: string;
  name: string;
  slug: string;
  active: boolean;
...
interface EditableAdminCategory {
  id: string;
  name: string;
  slug: string;
...
interface UpdateAdminCategoryFormProps {
  category: EditableAdminCategory;
  areas: AdminCategoryAreaOption[];
}

const initialState: UpdateAdminCategoryState = {
  status: "idle",
};
...
export function UpdateAdminCategoryForm({
  category,
  areas,
}: UpdateAdminCategoryFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-6">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-2 text-xs leading-5 text-amber-700">
              Cambiar el slug modifica la dirección pública de la categoría.
            </p>
      ...
      <section className="rounded-2xl border border-s-200 bg-white p-6 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-950">
          Áreas relacionadas
        </h2>

        <p className="mt-1 text-sm leading-6 text-slate-500">
          Selecciona las áreas en las que debe aparecer esta categoría.
        </p>

        {areas.length > 0 ? (
          <fieldset className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <legend className="sr-only">Seleccionar áreas</legend>

            {areas.map((area) => {
              const isRelated = category.areaIds.includes(area.id);

              const canSelect = area.active || isRelated;
              ...
              return (
                <label
                  ...
                  className={[
                    ...
                    canSelect
                      ? "cursor-pointer border-slate-200 hover:border-sky-300 hover:bg-sky-50/50"
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    {!area.active && isRelated && (
                      <span className="mt-1 block text-xs text-amber-700">
                        Esta relación se conservará mientras permanezca
                        seleccionada.
                      </span>
                    )}
                  </span>
        ...
        <FieldError errors={state.fieldErrors?.areaIds} />
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm leading-6 text-slate-500">
          Actualiza la información que utilizarán los buscadores.
        </p>
        ...
        <label className="inline-flex items-center gap-3">
          ...
          <span>
            ...
            <span className="block text-xs text-slate-500">
              Las categorías inactivas no se ofrecen para nuevos productos.
            </span>
          </span>
        </label>
