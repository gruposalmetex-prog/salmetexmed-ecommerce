...
function containsText(column: SQLWrapper, pattern: string): SQL {
  return sql`
...
function createAdminProductConditions(filters: AdminProductListFilters) {
  ...
  if (searchTerm) {
    ...
    const searchCondition = or(
      containsText(productsTable.name, pattern),

      containsText(productsTable.slug, pattern),
      ...
      exists(
        db
          ...
          .where(
            and(
              eq(brands.id, productsTable.brandId),

              or(
                containsText(brands.name, pattern),
                containsText(brands.slug, pattern),
              ),
            ),
          ),
      ),
        ...
        db
          ...
          .where(
            and(
              ...
              or(
                containsText(productVariants.name, pattern),
                containsText(productVariants.sku, pattern),
                containsText(productVariants.model, pattern),
              ),
            ),
          ),
      ),
...
export async function listAdminProducts(filters: AdminProductListFilters) {
  ...
  const whereCondition = createAdminProductConditions(filters);
...
export async function getAdminProductCounts() {
  ...
  const [statusRows, variantRows] = await Promise.all([
    db
      .select({
  ...
  return {
    statusRows,
    variantCount: Number(variantRows[0]?.count) || 0,
  };
}
...
export async function updateAdminProductRecord(input: UpdateAdminProductInput) {
  const db = getDatabase();
...
export async function insertAdminProductVariant(
  ...
  return db.transaction(async (transaction) => {
    ...
    if (!variant) {
      throw new Error("No fue posible crear la variante.");
    }
    return variant;
  });
}

export async function updateAdminProductVariantRecord(
  ...
  return db.transaction(async (transaction) => {
    ...
    const shouldBeDefault = currentVariant.isDefault || input.isDefault;

    if (input.isDefault && !currentVariant.isDefault) {
      await transaction
        .update(productVariants)
        .set({
          isDefault: false,
...
export type AdminProductListRecord = Awaited<
