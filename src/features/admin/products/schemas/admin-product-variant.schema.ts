...
const optionalPriceInCents = z.preprocess(
  (value) => {
    if (value === null || value === undefined) {
      return null;
    }
...
export const createAdminProductVariantSchema = z
  .object({
    ...
    sku: z
      ...
      .max(80, "El SKU no puede superar 80 caracteres")
    ...
    compareAtPriceInCents: optionalPriceInCents,
    ...
    stock: z.coerce
      .number({
        error: "Ingresa una existencia válida",
      })
      .int("La existencia debe ser un número entero")
      .min(0, "La existencia no puede ser negativa")
      .max(1_000_000, "La existencia supera el máximo permitido"),
  ...
  .superRefine((variant, context) => {
    if (variant.purchaseEnabled && variant.priceInCents === null) {
      context.addIssue({
        code: "custom",
        path: ["priceInCents"],
        message: "Una variante disponible para compra necesita precio",
      });
    }
    ...
    if (
      ...
      context.addIssue({
        ...
        message: "Primero debes indicar el precio de venta",
      });
    }
...
export type CreateAdminProductVariantInput = z.infer<
  typeof createAdminProductVariantSchema
>;

export const updateAdminProductVariantSchema = z.intersection(
  createAdminProductVariantSchema,

  z.object({
    variantId: z.uuid("La variante seleccionada no es válida"),
  }),
);

export type UpdateAdminProductVariantInput = z.infer<
  typeof updateAdminProductVariantSchema
>;
