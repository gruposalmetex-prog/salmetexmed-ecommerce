...
export async function getAdminDashboardSummary() {
    ...
    for (
        const row of result.statusRows
    ) {
        counts[row.status] =
            Number(row.count) || 0;
    }
...
export class AdminProductUpdateError
    extends Error {
    constructor(
        public readonly code:
            AdminProductUpdateErrorCode,
...
export async function updateAdminProduct(
    ...
    const updatedProduct =
        await updateAdminProductRecord(
            input,
        );
...
export class AdminProductVariantError
    extends Error {
    constructor(
        public readonly code:
            AdminProductVariantErrorCode,
...
export async function createAdminProductVariant(
    ...
    const product =
        await findAdminProductVariantContext(
            input.productId,
        );
    ...
    if (
        ...
        throw new AdminProductVariantError(
            "product_not_allowed",
            "Solo los productos de compra directa pueden habilitar variantes para compra.",
        );
    }
...
export function getAdminProductPublicationReadiness(
    ...
    const requiresPurchasableVariant =
        product.saleMode ===
        "direct_purchase";
        ...
        [
            {
                ...
                description:
                    "Nombre, slug, descripciones y categoría.",
            },
            {
                key: "variant",

                label:
                    requiresPurchasableVariant
...
export class AdminProductPublicationError
    extends Error {
    constructor(
        public readonly code:
            AdminProductPublicationErrorCode,

        message: string,

        public readonly missingRequirements:
            string[] = [],
    ) {
        super(message);
...
export async function publishAdminProduct(
    ...
    const publishedProduct =
        await markAdminProductAsPublished(
            product.id,
        );
...
export async function unpublishAdminProduct(
    ...
    const draftProduct = await markPublishedAdminProductAsDraft(
        product.id
    )
...
export type AdminDashboardSummary =
    Awaited<
        ReturnType<
            typeof getAdminDashboardSummary
        >
    >;
